...
*Zuletzt aktualisiert: 2026-08-17 — komplett gegen den korrigierten Stand von `Notebooks/02_Analyse.ipynb` (39 Zellen, echte Ausgaben nach Neuausführung gegen die korrigierte `Data/analysetabelle.csv`) abgeglichen. Grund der Korrektur: `QSErgBewStrukDialog` war in `01_Exploration.ipynb` invertiert interpretiert (R10 fälschlich als „auffällig" statt „nicht auffällig"). Alle Grafik-Befunde, T-Tests, Chi²-Test, ANOVA, Konfidenzintervalle und die Pivot-Tabelle neu berechnet und mit den tatsächlichen neuen Werten aktualisiert — mehrere Befunde (Bettenzahl, Ärzte/Bett, Pflege/Bett, Trägerschaft, alle Korrelationen) haben dabei die Richtung gewechselt. Zusätzlich am 2026-08-17: `scripts/Grafiken_Speichern.py` wiederhergestellt und alle 12 PNG-Grafiken in `grafiken/` neu erzeugt (zeigten zuvor noch den alten, unkorrigierten Stand).*
